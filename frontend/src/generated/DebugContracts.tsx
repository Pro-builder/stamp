...
import { useInbox_CloseInbox } from './hooks';



import { useInbox_Decline } from './hooks';



import { useInbox_OpenInbox } from './hooks';



import { useInbox_Reclaim } from './hooks';



import { useInbox_Reply } from './hooks';



import { useInbox_SendMessage } from './hooks';



import { useInbox_GetBoxCount } from './hooks';



import { useInbox_GetInbox } from './hooks';



import { useInbox_GetLastId } from './hooks';



import { useInbox_GetMessage } from './hooks';



import { useInbox_GetPage } from './hooks';
...
export default function DebugContracts() {
  ...
  const [openContract, setOpenContract] = useState<string | null>("inbox");
    ...
    <div style={S.page}>
      ...
      <div style={S.panel}>
        <button 
          style={S.panelBtn} 
          onClick={() => {
            setOpenContract(openContract === "inbox" ? null : "inbox");
        ...
        >
          <div style={Object.assign({ display: 'flex', alignItems: 'center' })}>
            ...
            <span style={S.panelName}>Inbox</span>
            <span style={S.panelExt}>.clar</span>
          </div>
          <span style={S.headerTag}>{openContract === "inbox" ? '▲' : '▼'}</span>
        </button>
        ...
        {openContract === "inbox" && (
          <div style={Object.assign({ padding: '20px', width:'100%' })} className='space-y-4'>
                ...
                {activeTab === 'write' && (
                  <FunctionCard_Inbox_CloseInbox 
                    num="04"
                    isOpen={openFunction === "inbox_close-inbox"}
                    onToggle={() => setOpenFunction(openFunction === "inbox_close-inbox" ? null : "inbox_close-inbox")}
                  />
                )}
                ...
                {activeTab === 'write' && (
                  <FunctionCard_Inbox_Decline 
                    num="05"
                    isOpen={openFunction === "inbox_decline"}
                    onToggle={() => setOpenFunction(openFunction === "inbox_decline" ? null : "inbox_decline")}
                  />
                )}
                ...
                {activeTab === 'write' && (
                  <FunctionCard_Inbox_OpenInbox 
                    num="06"
                    isOpen={openFunction === "inbox_open-inbox"}
                    onToggle={() => setOpenFunction(openFunction === "inbox_open-inbox" ? null : "inbox_open-inbox")}
                  />
                )}
                ...
                {activeTab === 'write' && (
                  <FunctionCard_Inbox_Reclaim 
                    num="07"
                    isOpen={openFunction === "inbox_reclaim"}
                    onToggle={() => setOpenFunction(openFunction === "inbox_reclaim" ? null : "inbox_reclaim")}
                  />
                )}
                ...
                {activeTab === 'write' && (
                  <FunctionCard_Inbox_Reply 
                    num="08"
                    isOpen={openFunction === "inbox_reply"}
                    onToggle={() => setOpenFunction(openFunction === "inbox_reply" ? null : "inbox_reply")}
                  />
                )}
                ...
                {activeTab === 'write' && (
                  <FunctionCard_Inbox_SendMessage 
                    num="09"
                    isOpen={openFunction === "inbox_send-message"}
                    onToggle={() => setOpenFunction(openFunction === "inbox_send-message" ? null : "inbox_send-message")}
                  />
                )}
  ...
  {activeTab === 'read' && (
    <FunctionCard_Inbox_GetBoxCount 
      num="10"
      isOpen={openFunction === "inbox_get-box-count"}
      onToggle={() => setOpenFunction(openFunction === "inbox_get-box-count" ? null : "inbox_get-box-count")}
    />
  )}
  ...
  {activeTab === 'read' && (
    <FunctionCard_Inbox_GetInbox 
      num="11"
      isOpen={openFunction === "inbox_get-inbox"}
      onToggle={() => setOpenFunction(openFunction === "inbox_get-inbox" ? null : "inbox_get-inbox")}
    />
  )}
  ...
  {activeTab === 'read' && (
    <FunctionCard_Inbox_GetLastId 
      num="12"
      isOpen={openFunction === "inbox_get-last-id"}
      onToggle={() => setOpenFunction(openFunction === "inbox_get-last-id" ? null : "inbox_get-last-id")}
    />
  )}
  ...
  {activeTab === 'read' && (
    <FunctionCard_Inbox_GetMessage 
      num="13"
      isOpen={openFunction === "inbox_get-message"}
      onToggle={() => setOpenFunction(openFunction === "inbox_get-message" ? null : "inbox_get-message")}
    />
  )}
  ...
  {activeTab === 'read' && (
    <FunctionCard_Inbox_GetPage 
      num="14"
      isOpen={openFunction === "inbox_get-page"}
      onToggle={() => setOpenFunction(openFunction === "inbox_get-page" ? null : "inbox_get-page")}
    />
  )}
...
function FunctionCard_Inbox_CloseInbox({ num, isOpen, onToggle }: { num: string, isOpen: boolean, onToggle: () => void }) {
  const { data, loading, txid, txStatus, txStatusError, explorerUrl, call } = useInbox_CloseInbox();
    ...
    <div style={Object.assign({}, S.card, { border: isOpen ? '1px solid #FF550E' : S.card.border })}>
      <div style={Object.assign({ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' })} onClick={onToggle}>
        <div style={Object.assign({ display: 'flex', alignItems: 'center' })}>
          ...
          <span style={S.fnName}>CLOSE-INBOX</span>
        </div>
...
function FunctionCard_Inbox_Decline({ num, isOpen, onToggle }: { num: string, isOpen: boolean, onToggle: () => void }) {
  const { data, loading, txid, txStatus, txStatusError, explorerUrl, call } = useInbox_Decline();
  const isReadOnly = false;
  ...
  return (
    <div style={Object.assign({}, S.card, { border: isOpen ? '1px solid #FF550E' : S.card.border })}>
      <div style={Object.assign({ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' })} onClick={onToggle}>
        <div style={Object.assign({ display: 'flex', alignItems: 'center' })}>
          <span style={S.fnNumber}>{num}</span>
          <span style={S.fnName}>DECLINE</span>
        </div>
        <div style={isReadOnly ? S.badgeRead : S.badgeWrite}>
          <div style={S.badgeDot} />
          {isReadOnly ? 'READ' : 'WRITE'}
      ...
      <div style={Object.assign({}, S.formContainer, { maxHeight: isOpen ? '1000px' : '0px', opacity: isOpen ? 1 : 0, marginTop: isOpen ? '20px' : '0px' })}>
        <form style={S.form} onSubmit={async (e) => { e.preventDefault(); await call([toClarityValue(args['id'] ?? '', "uint128", 'id'),]); }}>
          
            
            <div>
              <label style={S.argLabel}>id <span style={S.argType}>UINT128</span></label>
              
              <input style={S.argInput} value={args['id']} onChange={e => setArg('id', e.target.value)} />
...
function FunctionCard_Inbox_OpenInbox({ num, isOpen, onToggle }: { num: string, isOpen: boolean, onToggle: () => void }) {
  const { data, loading, txid, txStatus, txStatusError, explorerUrl, call } = useInbox_OpenInbox();
  const isReadOnly = false;
  ...
  return (
    <div style={Object.assign({}, S.card, { border: isOpen ? '1px solid #FF550E' : S.card.border })}>
      <div style={Object.assign({ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' })} onClick={onToggle}>
        <div style={Object.assign({ display: 'flex', alignItems: 'center' })}>
          <span style={S.fnNumber}>{num}</span>
          <span style={S.fnName}>OPEN-INBOX</span>
        </div>
        <div style={isReadOnly ? S.badgeRead : S.badgeWrite}>
          <div style={S.badgeDot} />
          {isReadOnly ? 'READ' : 'WRITE'}
      ...
      <div style={Object.assign({}, S.formContainer, { maxHeight: isOpen ? '1000px' : '0px', opacity: isOpen ? 1 : 0, marginTop: isOpen ? '20px' : '0px' })}>
        <form style={S.form} onSubmit={async (e) => { e.preventDefault(); await call([toClarityValue(args['price'] ?? '', "uint128", 'price'),toClarityValue(args['window'] ?? '', "uint128", 'window'),]); }}>
          
            
            <div>
              <label style={S.argLabel}>price <span style={S.argType}>UINT128</span></label>
              
              <input style={S.argInput} value={args['price']} onChange={e => setArg('price', e.target.value)} />
              
            </div>
            
            <div>
              <label style={S.argLabel}>window <span style={S.argType}>UINT128</span></label>
              
              <input style={S.argInput} value={args['window']} onChange={e => setArg('window', e.target.value)} />
...
function FunctionCard_Inbox_Reclaim({ num, isOpen, onToggle }: { num: string, isOpen: boolean, onToggle: () => void }) {
  const { data, loading, txid, txStatus, txStatusError, explorerUrl, call } = useInbox_Reclaim();
  const isReadOnly = false;
  ...
  return (
    <div style={Object.assign({}, S.card, { border: isOpen ? '1px solid #FF550E' : S.card.border })}>
      <div style={Object.assign({ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' })} onClick={onToggle}>
        <div style={Object.assign({ display: 'flex', alignItems: 'center' })}>
          <span style={S.fnNumber}>{num}</span>
          <span style={S.fnName}>RECLAIM</span>
        </div>
        <div style={isReadOnly ? S.badgeRead : S.badgeWrite}>
          <div style={S.badgeDot} />
          {isReadOnly ? 'READ' : 'WRITE'}
...
function FunctionCard_Inbox_Reply({ num, isOpen, onToggle }: { num: string, isOpen: boolean, onToggle: () => void }) {
  const { data, loading, txid, txStatus, txStatusError, explorerUrl, call } = useInbox_Reply();
  const isReadOnly = false;
  ...
  return (
    <div style={Object.assign({}, S.card, { border: isOpen ? '1px solid #FF550E' : S.card.border })}>
      <div style={Object.assign({ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' })} onClick={onToggle}>
        <div style={Object.assign({ display: 'flex', alignItems: 'center' })}>
          <span style={S.fnNumber}>{num}</span>
          <span style={S.fnName}>REPLY</span>
        </div>
        <div style={isReadOnly ? S.badgeRead : S.badgeWrite}>
          <div style={S.badgeDot} />
          {isReadOnly ? 'READ' : 'WRITE'}
      ...
      <div style={Object.assign({}, S.formContainer, { maxHeight: isOpen ? '1000px' : '0px', opacity: isOpen ? 1 : 0, marginTop: isOpen ? '20px' : '0px' })}>
        <form style={S.form} onSubmit={async (e) => { e.preventDefault(); await call([toClarityValue(args['id'] ?? '', "uint128", 'id'),toClarityValue(args['body'] ?? '', {"string-utf8":{"length":280}}, 'body'),]); }}>
            ...
            <div>
              <label style={S.argLabel}>body <span style={S.argType}>STRING-UTF8</span></label>
              
              <input style={S.argInput} value={args['body']} onChange={e => setArg('body', e.target.value)} />
...
function FunctionCard_Inbox_SendMessage({ num, isOpen, onToggle }: { num: string, isOpen: boolean, onToggle: () => void }) {
  const { data, loading, txid, txStatus, txStatusError, explorerUrl, call } = useInbox_SendMessage();
  const isReadOnly = false;

  
  const [args, setArgs] = useState<Record<string, string>>({
    'to': '', 'body': '', 'expected-price': '', 
  });
  const setArg = (name: string, value: string) => setArgs(a => Object.assign({}, a, { [name]: value }));
  ...
  return (
    <div style={Object.assign({}, S.card, { border: isOpen ? '1px solid #FF550E' : S.card.border })}>
      <div style={Object.assign({ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' })} onClick={onToggle}>
        <div style={Object.assign({ display: 'flex', alignItems: 'center' })}>
          <span style={S.fnNumber}>{num}</span>
          <span style={S.fnName}>SEND-MESSAGE</span>
        </div>
        <div style={isReadOnly ? S.badgeRead : S.badgeWrite}>
          <div style={S.badgeDot} />
          {isReadOnly ? 'READ' : 'WRITE'}
      ...
      <div style={Object.assign({}, S.formContainer, { maxHeight: isOpen ? '1000px' : '0px', opacity: isOpen ? 1 : 0, marginTop: isOpen ? '20px' : '0px' })}>
        <form style={S.form} onSubmit={async (e) => { e.preventDefault(); await call([toClarityValue(args['to'] ?? '', "principal", 'to'),toClarityValue(args['body'] ?? '', {"string-utf8":{"length":280}}, 'body'),toClarityValue(args['expected-price'] ?? '', "uint128", 'expected-price'),]); }}>
          
            
            <div>
              <label style={S.argLabel}>to <span style={S.argType}>PRINCIPAL</span></label>
              
              <input style={S.argInput} value={args['to']} onChange={e => setArg('to', e.target.value)} />
            ...
            <div>
              <label style={S.argLabel}>expected-price <span style={S.argType}>UINT128</span></label>
              
              <input style={S.argInput} value={args['expected-price']} onChange={e => setArg('expected-price', e.target.value)} />
...
function FunctionCard_Inbox_GetBoxCount({ num, isOpen, onToggle }: { num: string, isOpen: boolean, onToggle: () => void }) {
  const { data, loading, txid, txStatus, txStatusError, explorerUrl, call } = useInbox_GetBoxCount();
  ...
  return (
    <div style={Object.assign({}, S.card, { border: isOpen ? '1px solid #FF550E' : S.card.border })}>
      <div style={Object.assign({ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' })} onClick={onToggle}>
        <div style={Object.assign({ display: 'flex', alignItems: 'center' })}>
          <span style={S.fnNumber}>{num}</span>
          <span style={S.fnName}>GET-BOX-COUNT</span>
        </div>
        <div style={isReadOnly ? S.badgeRead : S.badgeWrite}>
          <div style={S.badgeDot} />
          {isReadOnly ? 'READ' : 'WRITE'}
      ...
      <div style={Object.assign({}, S.formContainer, { maxHeight: isOpen ? '1000px' : '0px', opacity: isOpen ? 1 : 0, marginTop: isOpen ? '20px' : '0px' })}>
        <form style={S.form} onSubmit={async (e) => { e.preventDefault(); await call([toClarityValue(args['owner'] ?? '', "principal", 'owner'),toClarityValue(args['box'] ?? '', "uint128", 'box'),]); }}>
            ...
            <div>
              <label style={S.argLabel}>box <span style={S.argType}>UINT128</span></label>
              
              <input style={S.argInput} value={args['box']} onChange={e => setArg('box', e.target.value)} />
...
function FunctionCard_Inbox_GetInbox({ num, isOpen, onToggle }: { num: string, isOpen: boolean, onToggle: () => void }) {
  const { data, loading, txid, txStatus, txStatusError, explorerUrl, call } = useInbox_GetInbox();
  const isReadOnly = true;
  ...
  return (
    <div style={Object.assign({}, S.card, { border: isOpen ? '1px solid #FF550E' : S.card.border })}>
      <div style={Object.assign({ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' })} onClick={onToggle}>
        <div style={Object.assign({ display: 'flex', alignItems: 'center' })}>
          <span style={S.fnNumber}>{num}</span>
          <span style={S.fnName}>GET-INBOX</span>
        </div>
        <div style={isReadOnly ? S.badgeRead : S.badgeWrite}>
          <div style={S.badgeDot} />
          {isReadOnly ? 'READ' : 'WRITE'}
...
function FunctionCard_Inbox_GetLastId({ num, isOpen, onToggle }: { num: string, isOpen: boolean, onToggle: () => void }) {
  const { data, loading, txid, txStatus, txStatusError, explorerUrl, call } = useInbox_GetLastId();
  const isReadOnly = true;
  ...
  return (
    <div style={Object.assign({}, S.card, { border: isOpen ? '1px solid #FF550E' : S.card.border })}>
      <div style={Object.assign({ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' })} onClick={onToggle}>
        <div style={Object.assign({ display: 'flex', alignItems: 'center' })}>
          <span style={S.fnNumber}>{num}</span>
          <span style={S.fnName}>GET-LAST-ID</span>
        </div>
        <div style={isReadOnly ? S.badgeRead : S.badgeWrite}>
          <div style={S.badgeDot} />
          {isReadOnly ? 'READ' : 'WRITE'}
...
function FunctionCard_Inbox_GetMessage({ num, isOpen, onToggle }: { num: string, isOpen: boolean, onToggle: () => void }) {
  const { data, loading, txid, txStatus, txStatusError, explorerUrl, call } = useInbox_GetMessage();
  const isReadOnly = true;
  ...
  return (
    <div style={Object.assign({}, S.card, { border: isOpen ? '1px solid #FF550E' : S.card.border })}>
      <div style={Object.assign({ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' })} onClick={onToggle}>
        <div style={Object.assign({ display: 'flex', alignItems: 'center' })}>
          <span style={S.fnNumber}>{num}</span>
          <span style={S.fnName}>GET-MESSAGE</span>
        </div>
        <div style={isReadOnly ? S.badgeRead : S.badgeWrite}>
          <div style={S.badgeDot} />
          {isReadOnly ? 'READ' : 'WRITE'}
...
function FunctionCard_Inbox_GetPage({ num, isOpen, onToggle }: { num: string, isOpen: boolean, onToggle: () => void }) {
  const { data, loading, txid, txStatus, txStatusError, explorerUrl, call } = useInbox_GetPage();
  const isReadOnly = true;
  ...
  return (
    <div style={Object.assign({}, S.card, { border: isOpen ? '1px solid #FF550E' : S.card.border })}>
      <div style={Object.assign({ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' })} onClick={onToggle}>
        <div style={Object.assign({ display: 'flex', alignItems: 'center' })}>
          <span style={S.fnNumber}>{num}</span>
          <span style={S.fnName}>GET-PAGE</span>
        </div>
        <div style={isReadOnly ? S.badgeRead : S.badgeWrite}>
          <div style={S.badgeDot} />
          {isReadOnly ? 'READ' : 'WRITE'}
      ...
      <div style={Object.assign({}, S.formContainer, { maxHeight: isOpen ? '1000px' : '0px', opacity: isOpen ? 1 : 0, marginTop: isOpen ? '20px' : '0px' })}>
        <form style={S.form} onSubmit={async (e) => { e.preventDefault(); await call([toClarityValue(args['owner'] ?? '', "principal", 'owner'),toClarityValue(args['box'] ?? '', "uint128", 'box'),toClarityValue(args['offset'] ?? '', "uint128", 'offset'),]); }}>
            ...
            <div>
              <label style={S.argLabel}>offset <span style={S.argType}>UINT128</span></label>
              
              <input style={S.argInput} value={args['offset']} onChange={e => setArg('offset', e.target.value)} />
